...
var qrcode = function () {
  var qrcode = function (typeNumber, errorCorrectionLevel) {
    ...
    _this.make = function () {
      if (_typeNumber < 1) {
        ...
        for (; typeNumber < 40; typeNumber++) {
          ...
          var buffer = qrBitBuffer(), totalDataCount = 0;
    ...
    _this.createSvgTag = function (cellSize, margin) {
      cellSize = cellSize || 2; margin = (typeof margin == 'undefined') ? cellSize * 0 : margin;
      var size = _moduleCount * cellSize + margin * 2;
      var qrSvg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + size + ' ' + size + '" shape-rendering="crispEdges">';
      qrSvg += '<rect width="100%" height="100%" fill="#ffffff"/>';
      qrSvg += '<path d="';
      for (var row = 0; row < _moduleCount; row += 1) {
        var mr = row * cellSize + margin;
        for (var col = 0; col < _moduleCount; col += 1) {
          if (_this.isDark(row, col)) { var mc = col * cellSize + margin; qrSvg += 'M' + mc + ',' + mr + 'h' + cellSize + 'v' + cellSize + 'h-' + cellSize + 'z '; }
        }
      }
      qrSvg += '" fill="#15110c"/></svg>';
      return qrSvg;
    };
    return _this;
  };
  ...
  var QRUtil = (function () {
    ...
    _this.getLostPoint = function (qrcode) {
      ...
      for (var row = 0; row < moduleCount; row += 1) for (var col = 0; col < moduleCount; col += 1) {
        ...
        for (var r = -1; r <= 1; r += 1) { if (row + r < 0 || moduleCount <= row + r) continue; for (var c = -1; c <= 1; c += 1) { if (col + c < 0 || moduleCount <= col + c) continue; if (r == 0 && c == 0) continue; if (dark == qrcode.isDark(row + r, col + c)) sameCount += 1; } }
  ...
  function qrPolynomial(num, shift) {
    ...
    var _num = (function () { var offset = 0; while (offset < num.length && num[offset] == 0) offset += 1; var n = new Array(num.length - offset + shift); for (var i = 0; i < num.length - offset; i += 1) n[i] = num[i + offset]; return n; })();
    var _this = {};
  ...
  var QRRSBlock = (function () {
    var RS_BLOCK_TABLE = [[1, 26, 19], [1, 26, 16], [1, 26, 13], [1, 26, 9], [1, 44, 34], [1, 44, 28], [1, 44, 22], [1, 44, 16], [1, 70, 55], [1, 70, 44], [2, 35, 17], [2, 35, 13], [1, 100, 80], [2, 50, 32], [2, 50, 24], [4, 25, 9], [1, 134, 108], [2, 67, 43], [2, 33, 15, 2, 34, 16], [2, 33, 11, 2, 34, 12], [2, 86, 68], [4, 43, 27], [4, 43, 19], [4, 43, 15], [2, 98, 78], [4, 49, 31], [2, 32, 14, 4, 33, 15], [4, 39, 13, 1, 40, 14], [2, 121, 97], [2, 60, 38, 2, 61, 39], [4, 40, 18, 2, 41, 19], [4, 40, 14, 2, 41, 15], [2, 146, 116], [3, 58, 36, 2, 59, 37], [4, 36, 16, 4, 37, 17], [4, 36, 12, 4, 37, 13], [2, 86, 68, 2, 87, 69], [4, 69, 43, 1, 70, 44], [6, 43, 19, 2, 44, 20], [6, 43, 15, 2, 44, 16], [4, 101, 81], [1, 80, 50, 4, 81, 51], [4, 50, 22, 4, 51, 23], [3, 36, 12, 8, 37, 13], [2, 116, 92, 2, 117, 93], [6, 58, 36, 2, 59, 37], [4, 46, 20, 6, 47, 21], [7, 42, 14, 4, 43, 15], [4, 133, 107, 1, 134, 108]];
    ...
    _this.getRSBlocks = function (typeNumber, errorCorrectionLevel) {
      ...
      for (var i = 0; i < length; i += 1) { var count = rsBlock[i * 3 + 0], totalCount = rsBlock[i * 3 + 1], dataCount = rsBlock[i * 3 + 2]; for (var j = 0; j < count; j += 1) list.push(qrRSBlock(totalCount, dataCount)); }
      return list;
    };
    return _this;
  })();
  ...
  function qr8BitByte(data) {
    var _mode = QRMode.MODE_8BIT_BYTE, _bytes = (function (s) { var bytes = []; for (var i = 0; i < s.length; i++) { var c = s.charCodeAt(i); if (c < 0x80) bytes.push(c); else if (c < 0x800) { bytes.push(0xc0 | (c >> 6)); bytes.push(0x80 | (c & 0x3f)); } else if (c < 0xd800 || c >= 0xe000) { bytes.push(0xe0 | (c >> 12)); bytes.push(0x80 | ((c >> 6) & 0x3f)); bytes.push(0x80 | (c & 0x3f)); } else { i++; var cc = 0x10000 + (((c & 0x3ff) << 10) | (s.charCodeAt(i) & 0x3ff)); bytes.push(0xf0 | (cc >> 18)); bytes.push(0x80 | ((cc >> 12) & 0x3f)); bytes.push(0x80 | ((cc >> 6) & 0x3f)); bytes.push(0x80 | (cc & 0x3f)); } } return bytes; })(data);
    var _this = {};
  ...
  function qrNumber(data) { var _mode = QRMode.MODE_NUMBER, _this = {}; _this.getMode = function () { return _mode; }; _this.getLength = function () { return data.length; }; _this.write = function (buffer) { var i = 0; while (i + 2 < data.length) { buffer.put(parseInt(data.substring(i, i + 3), 10), 10); i += 3; } if (i < data.length) { if (data.length - i == 1) buffer.put(parseInt(data.substring(i, i + 1), 10), 4); else if (data.length - i == 2) buffer.put(parseInt(data.substring(i, i + 2), 10), 7); } }; return _this; }
  function qrAlphaNum(data) { var _mode = QRMode.MODE_ALPHA_NUM, _this = {}; var getCode = function (c) { if ('0' <= c && c <= '9') return c.charCodeAt(0) - '0'.charCodeAt(0); else if ('A' <= c && c <= 'Z') return c.charCodeAt(0) - 'A'.charCodeAt(0) + 10; else switch (c) { case ' ': return 36; case '$': return 37; case '%': return 38; case '*': return 39; case '+': return 40; case '-': return 41; case '.': return 42; case '/': return 43; case ':': return 44; default: throw 'illegal char :' + c; } }; _this.getMode = function () { return _mode; }; _this.getLength = function () { return data.length; }; _this.write = function (buffer) { var i = 0; while (i + 1 < data.length) { buffer.put(getCode(data.charAt(i)) * 45 + getCode(data.charAt(i + 1)), 11); i += 2; } if (i < data.length) buffer.put(getCode(data.charAt(i)), 6); }; return _this; }
  function qrKanji() { throw 'Kanji mode not supported'; }
